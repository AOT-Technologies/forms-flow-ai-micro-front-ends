...
The `RSBCImage` instance listens for the `triggerPrint` event. To trigger the print function, the component must send a `triggerPrint` event.
**Example Usage:**
```javascript
  form.getComponent("rsbcImage456").emit("triggerPrint", { data: "YES" });
```
