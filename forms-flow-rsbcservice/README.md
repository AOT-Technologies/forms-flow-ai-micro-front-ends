...
### Retrieving Base64 Images
You can get the Base64-encoded images generated by the RSBCImage component by calling the getBase64Images() function. This function is asynchronous and returns a dictionary of image keys mapped to their Base64 string values.
```javascript
  const rsbcImageComponent = form.getComponent('rsbcImageStageTwo');
  rsbcImageComponent.getBase64Images("stageTwo").then((images)=>{
        console.log('Base64Images:'+JSON.stringify(images,null,2));
      });
```
Where `rsbcImageStageTwo` is the API Property Name of the RSBCImage instance.

### Triggering Print Function
The `RSBCImage` instance provides two functions for triggering the print functionality:
-  `handlePrint` – Displays confirmation dialogs (detailed below) that emit events.
-  `handleSubmissionPrint` – Bypasses the confirmation dialogs and prints immediately.
**Example Usage:**
```javascript
  form.getComponent("rsbcImage456").handlePrint();  //Displays the confirmation dialogs
  form.getComponent("rsbcImage456").handleSubmissionPrint(); //Prints without confirmation dialogs
```
Where `rsbcImage456` is the API Property Name of the RSBCImage instance.
