  ...
  if (Object.keys(values).length) {
    if (renderStage === "stageTwo") {
      if (formLayout === "TwelveHour") {
        ...
        svgStyle = isPreview
            ? { marginLeft: "0px", marginRight: "0px", marginTop: "50px", marginBottom: "40px" }
            : { marginLeft: "-430px", marginRight: "-280px", marginTop: "50px", marginBottom: "40px" };
      }
    }
          ...
          <svg viewBox={viewBox} xmlns="http://www.w3.org/2000/svg" className={"svg-wrapper" + formAspect}>
            ...
            {formFields?.map((item: string) => {
              ...
              if (field["field_type"] === "text") {
                return (
                    <text key={fieldKey} id={item} x={`${field["start"]["x"]}px`} y={`${field["start"]["y"]}px`} className={field["classNames"]} fill="black">
                      {printFormatHelper(values, field, item, impoundLotOperators)}
                    </text>
                );
              } else if (field["field_type"] === "checkbox") {
                return (
                    <text key={fieldKey} id={item} x={field["start"]["x"]} y={field["start"]["y"]} className={field["classNames"]}>
                ...
                return (
                    <foreignObject key={fieldKey} id={item} x={`${field["start"]["x"]}px`} y={`${field["start"]["y"]}px`} className={field["classNames"]} fill="black">
                      {printFormatHelper(values, field, item, impoundLotOperators)}
                    </foreignObject>
                );
              }
