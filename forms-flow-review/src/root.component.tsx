...
export default function Root(props: any) {
  ...
  // Lazy initializer: create the store exactly once per mount. Building it in
  // the component body would create a fresh store (wiping all state) on every
  // re-render of Root.
  const [store] = useState(() => StoreService.configureStore());
  ...
  useEffect(() => {
    ...
    if (existingTaskData) {
      store.dispatch(
        resetTaskListParams({ ...existingTaskData, filterCached: true })
      );
    }
    ...
    return () => {
      ...
      if (isAuth) {
        StorageService.saveDataToSessionStorage(
          TASK_APP_KEY,
          store.getState().task
        );
      }
  ...
  return (
    <Provider store={store}>
      <BrowserRouter>
        <Routes>
          <Route path={`${BASE_ROUTE}*`} element={<Task {...props} />} />
        </Routes>
      </BrowserRouter>
    </Provider>
  );
