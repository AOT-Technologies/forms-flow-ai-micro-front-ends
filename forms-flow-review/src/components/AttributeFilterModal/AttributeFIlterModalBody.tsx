...
const VARIABLES_WITH_FORM_SUPPORT = new Set([
  "name",
  "submitterName",
  "assignee",
  "roles",
...
const AttributeFilterModalBody = ({
  onClose,
  ...
  const defaultTaskFilterId = useSelector(
    (state: any) => state.task.defaultFilter
  );
  ...
  const isQuickTaskFilterContext =
    isUnsavedFilter || (defaultTaskFilterId === null && !!selectedFilter);
  ...
  const processTaskFields = (existingValues) => {
    ...
    taskFields.forEach((fieldName) => {
      ...
      if (fieldValue) {
        const taskVariable = taskVariables.find(
          (tv) => tv.name === fieldName && !tv.isFormVariable
        );
        if (taskVariable) {
          const resetValue = cleanValue(
            fieldValue,
            fieldName,
            taskVariable.type
          );
  ...
  const processNameLikeField = (existingValues) => {
    ...
    if (nameLikeValue) {
      const taskNameVariable = taskVariables.find(
        (tv) => tv.name === "name" && !tv.isFormVariable
      );
      if (taskNameVariable) {
        const resetValue = cleanValue(
          nameLikeValue,
          "name",
          taskNameVariable.type
        );
  ...
  const [attributeData, setAttributeData] = useState(() => {
    const initialData = {
      ...
      roles:
        removeTenantKey(
          selectedAttributeFilter?.criteria?.candidateGroup,
          tenantKey,
  ...
  const [shareAttrFilter, setShareAttrFilter] = useState(
    FILTER_SHARE_OPTIONS.PRIVATE
  );
  ...
  useEffect(() => {
    if (!attributeFilter) {
      ...
      const initialData = {
        ...
        roles:
          removeTenantKey(
            selectedFilter?.criteria?.candidateGroup,
            tenantKey,
            MULTITENANCY_ENABLED
          ) || "",
      };
      ...
      const initialData = {
        ...
        roles:
          removeTenantKey(
            attributeFilter?.criteria?.candidateGroup,
            tenantKey,
            MULTITENANCY_ENABLED
          ) || "",
      };
      ...
      const attributeFilterProcessVars =
      ...
      attributeFilterProcessVars.forEach((item) => {
        ...
        if (VARIABLES_WITH_FORM_SUPPORT.has(item.name)) {
          const taskVariable = taskVariables.find(
            (tv) =>
              tv.name === item.name && tv.isFormVariable === item.isFormVariable
          );
          if (taskVariable) {
  ...
  const getCandidateGroup = () => {
    return (
      removeSlashFromValue(attributeData["roles"]) ||
      selectedFilter?.criteria?.candidateGroup
    );
  };
  ...
  const removeSlashFromValue = (value) => {
    ...
    return addTenantPrefixIfNeeded(
      trimmedValue,
      tenantKey,
      MULTITENANCY_ENABLED
    );
  ...
  const noFieldChanged = isUnsavedFilter
    ? false
    : isEqual(selectedAttributeFilter, createAttributeFilterPayload());
  ...
  const saveFilterTab = () => (
    <div className="save-filter-tab-container">
      {!attributeFilter?.id && (
        <Alert
          message={t(
            "You can easily edit or delete custom filters at any time after they are saved."
          )}
          variant="passive"
      ...
      <div className="custom-input-container">
        ...
        <SelectDropdown
          options={filterShareOptions.map(({ label, value }) => ({
            label,
            value,
          }))}
