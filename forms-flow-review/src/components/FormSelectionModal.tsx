...
export const FormSelectionModal: React.FC<FormSelectionModalProps> = React.memo(
  ({ onSelectForm, forms, selectedForm: selectedFormProp }) => {
    ...
    const [selectedForm, setSelectedForm] = useState({
      formId: selectedFormProp?.formId || "",
      formName: selectedFormProp?.formName || "",
    });
    ...
    useEffect(() => {
      if (
        selectedFormProp?.formId &&
        selectedFormProp.formId !== selectedForm.formId
      ) {
        setSelectedForm({
          formId: selectedFormProp.formId,
          formName: selectedFormProp.formName,
    ...
    function renderFormList() {
      ...
      if (formOptions.length > 0) {
        ...
        const items = formOptions.map((item) => ({
          label: item.formName,
          value: String(item.formId),
        }));
        const selectedValues = selectedForm.formId
          ? [String(selectedForm.formId)]
          : [];
        return (
          <CustomCheckbox
            ...
            onChange={(_values, event) => {
              ...
              const matched = formOptions.find(
                (i) => String(i.formId) === clickedId
              );
              if (matched) {
                const newSel = {
                  formId: String(matched.formId),
      ...
      return (
        <span className="nothing-found-text">
