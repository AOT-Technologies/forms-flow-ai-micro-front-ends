...
// Direct sibling import instead of the package's own root barrel, which
...
export const AutoVariableSelection: React.FC<AutoVariableSelectionProps> =
  React.memo(
    ({
      form,
      ...
      const [availableFormVariables, setAvailableFormVariables] = useState<
        Record<string, FormVariable>
      >({});
      // Store the currently selected variable (only one can be selected)
      const [selectedVariable, setSelectedVariable] =
        useState<FormVariable | null>(null);
      const detailsRef = useRef(null);
      const isInternalUpdateRef = useRef(false);
      const prevPropSelectedVariableRef = useRef<
        FormVariable | null | undefined
      ...
      const isLayoutComponent = useCallback(
        (component: any, parentType?: string): boolean => {
          const layoutTypes = [
            "panel",
            "columns",
            "fieldset",
          ...
          return (
            (component?.type && layoutTypes.includes(component.type)) ||
            parentType === "tabs"
          );
        },
        []
      ...
      const handleVariableSelect = useCallback((variable: FormVariable) => {
        ...
        if (variable.isFormVariable === false) {
          ...
          if (!component) {
            const allComponents =
              document.querySelectorAll(".formio-component");
            for (let i = 0; i < allComponents.length; i++) {
              const comp = allComponents[i];
              const classes = Array.from(comp.classList);
              const keyClass = classes.find((cls) =>
      ...
      const handleFormComponentClick = useCallback(
        (selected: {
          key: string | null;
          type: string;
          label: string;
                    ...
                    <div className="variable-list">
                      {systemVariables.map((sysVar) => {
                        const variable: FormVariable = {
                          key: sysVar.key,
                          altVariable:
                            sysVar.altVariable || sysVar.labelOfComponent,
