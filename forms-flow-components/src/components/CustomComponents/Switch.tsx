...
const SwitchComponent = forwardRef<HTMLDivElement, SwitchProps>(
  (
    ...
    const wrapperClassName = useMemo(() => {
      return buildClassNames("custom-switch-wrapper", className);
    }, [className]);
    ...
    const generateSecureId = useCallback(() => {
      try {
        ...
        console.warn(
          "Crypto API not available, falling back to timestamp:",
          error
        );
      }
    ...
    return (
      <div ref={ref} className={wrapperClassName} {...restProps}>
        {label && (
          <label
            htmlFor={effectiveId}
            id={labelId}
